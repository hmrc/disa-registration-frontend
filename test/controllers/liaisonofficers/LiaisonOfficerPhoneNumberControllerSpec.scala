...
class LiaisonOfficerPhoneNumberControllerSpec extends SpecBase {
  ...
  private val oldNumber    = "01234567890"
  private val newNumber    = "09876543210"

  lazy val routeUrl: String  = LiaisonOfficerPhoneNumberController.onPageLoad(existingId, NormalMode).url
  lazy val submitUrl: String = LiaisonOfficerPhoneNumberController.onSubmit(existingId, NormalMode).url
  ...
  "LiaisonOfficerPhoneNumberController" - {

    "must return OK and the correct view for a GET when the liaison officer exists and has no phone number" in {
      ...
      val journeyData =
        JourneyData(
          ...
          liaisonOfficers = Some(
            LiaisonOfficers(
              Seq(
                LiaisonOfficer(existingId, Some(existingName), None)
              )
            )
          )
        )
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[LiaisonOfficerPhoneNumberView]
    ...
    "must populate the view correctly on a GET when previously answered" in {
      ...
      val journeyData =
        JourneyData(
          ...
          liaisonOfficers = Some(
            LiaisonOfficers(
              Seq(
                LiaisonOfficer(otherId, Some("Other Person"), Some("01111111111")),
                LiaisonOfficer(existingId, Some(existingName), Some(oldNumber))
              )
            )
          )
        )
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(existingId, existingName, form.fill(oldNumber), NormalMode, None)(
          request,
          messages(application)
        ).toString
      }
      ...
      val journeyData =
        JourneyData(
          ...
          liaisonOfficers = Some(
            LiaisonOfficers(
              Seq(
                LiaisonOfficer(existingId, None, Some(oldNumber))
              )
            )
          )
        )
    ...
    "must redirect to the next page when valid data is submitted and existing liaison officer is updated" in {
      ...
      val expectedSection =
        LiaisonOfficers(
          Seq(
            LiaisonOfficer(otherId, Some("Other Person"), Some("01111111111")),
            LiaisonOfficer(existingId, Some(existingName), Some(newNumber))
          )
        )
      ...
      running(application) {
        val request =
          FakeRequest(POST, submitUrl)
            .withFormUrlEncodedBody(("value", newNumber))
    ...
    "must render view with CheckMode on a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, LiaisonOfficerPhoneNumberController.onPageLoad(existingId, CheckMode).url)
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(existingId, existingName, form.fill(oldNumber), CheckMode, None)(
          request,
          messages(application)
        ).toString
      }
    ...
    "must submit with CheckMode and redirect" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, LiaisonOfficerPhoneNumberController.onSubmit(existingId, CheckMode).url)
            .withFormUrlEncodedBody(("value", newNumber))
