...
class EligibilityToManageIsaControllerSpec extends SpecBase {
  ...
  "EligibilityToManageIsaControllerSpec" - {

    "must return OK and the correct view for a GET" in {

      val application = applicationBuilder(journeyData = Some(emptyJourneyData)).build()
      when(mockAppConfig.isaManagerGuidanceUrl).thenReturn("https://www.gov.uk/guidance/apply-to-be-an-isa-manager")

      running(application) {
        val request = FakeRequest(GET, routes.EligibilityToManageIsaController.onPageLoad().url)
