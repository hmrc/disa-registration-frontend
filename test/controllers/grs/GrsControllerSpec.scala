  ...
  "GrsController" - {
    ...
    "grsCallback" - {
      ...
      "must redirect to TaskList when both registration and verification pass" in {
        ...
        when(mockGrsService.fetchGRSJourneyData(eqTo(journeyId))(any(), any())).thenReturn(Future.successful(grsResponse))
        when(mockJourneyAnswersService.update(any[BusinessVerification], any())(any(), any())).thenReturn(Future.successful(BusinessVerification(Some(true), Some(true), Some("1234567890"))))
      ...
      "must redirect to Lockout when business verification fails" in {
        ...
        when(mockGrsService.fetchGRSJourneyData(eqTo(journeyId))(any(), any())).thenReturn(Future.successful(grsResponse))
        when(mockJourneyAnswersService.update(any[BusinessVerification], any())(any(), any())).thenReturn(Future.successful(BusinessVerification(Some(true), Some(false), Some("1234567890"))))
      ...
      "must redirect to Start when no business registration/verification data present)" in {
        ...
        when(mockGrsService.fetchGRSJourneyData(eqTo(journeyId))(any(), any())).thenReturn(Future.successful(grsResponse))
        when(mockJourneyAnswersService.update(any[BusinessVerification], any())(any(), any())).thenReturn(Future.successful(BusinessVerification(None, None, Some("1234567890"))))
      ...
      "must redirect to Start when business registration fails - (Not sure how this is possible in prod)" in {
        ...
        val grsResponse = baseGRSResponse(businessRegistrationStatus = FailedStatus)
        when(mockGrsService.fetchGRSJourneyData(eqTo(journeyId))(any(), any())).thenReturn(Future.successful(grsResponse))
        when(mockJourneyAnswersService.update(any[BusinessVerification], any())(any(), any())).thenReturn(Future.successful(BusinessVerification(Some(false), Some(true), Some("1234567890"))))
      ...
      "must propagate exception if journeyAnswersService fails" in {
        ...
        val grsResponse = baseGRSResponse()
        when(mockGrsService.fetchGRSJourneyData(eqTo(journeyId))(any(), any())).thenReturn(Future.successful(grsResponse))
        when(mockJourneyAnswersService.update(any[BusinessVerification], any())(any(), any())).thenReturn(Future.failed(new Exception("Update journeyAnswersService failed - Service Down")))
      ...
      "must propagate exception if grsService fails" in {
        ...
        when(mockGrsService.fetchGRSJourneyData(eqTo(journeyId))(any(), any())).thenReturn(Future.failed(new Exception("GRS failed - Service Down")))
