...
object SubmissionCyaViewModel {
    ...
    Seq(
      ...
      organisationDetails.flatMap(_.correspondenceAddress).flatMap { address =>
        addressRow(
          ...
          address = address,
          href = Some(RegisteredAddressCorrespondenceController.onPageLoad(CheckMode, returnTo).url),
  ...
  private def yesNo(answer: Boolean)(implicit messages: Messages): Value =
