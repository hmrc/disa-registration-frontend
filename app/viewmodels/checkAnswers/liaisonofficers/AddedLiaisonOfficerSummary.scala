...
class AddedLiaisonOfficersViewModel @Inject() (
  ...
  private def row(liaisonOfficer: LiaisonOfficer, returnTo: Option[ReturnTo])(implicit
    ...
    val changeLink =
      if (liaisonOfficer.inProgress)
        LiaisonOfficerNameController.onPageLoad(Some(liaisonOfficer.id), NormalMode, returnTo).url
      else LoCheckYourAnswersController.onPageLoad(liaisonOfficer.id, returnTo).url
