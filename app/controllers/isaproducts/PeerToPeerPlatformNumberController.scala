...
class PeerToPeerPlatformNumberController @Inject() (
          ...
          .fold(
            ...
            answer => {
              ...
              journeyAnswersService
                ...
                .map { _ =>
                  Redirect(navigator.nextPage(PeerToPeerPlatformNumberPage, mode))
                }
                .recoverWith { case e =>
                  logger.warn(
  ...
  private def withPrerequisites(
    ...
    val requiredData: Option[(IsaProducts, String)] =
      for {
        products <- request.journeyData.isaProducts
        name     <- products.p2pPlatform
      } yield (products, name)

    requiredData.fold[Future[Result]](errorHandler.badRequest)(data => f(data._1, data._2, formProvider(data._2)))
  }
}
