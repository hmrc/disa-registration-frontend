...
class DeclarationForIsaManagersController @Inject() (
  ...
  def onPageLoad: Action[AnyContent] = (identify andThen getData andThen requireData) { implicit request =>
    val showJuniorContent = request.journeyData.isaProducts.map(
      _.isaProducts.exists(answers => answers.contains(CashJuniorIsas) || answers.contains(StocksAndShareJuniorIsas))
    )
